...
class DebugOptionPage(QWizardPage):
    def __init__(self):
        ...
        self.comment_label = QLabel("BUILD_LEVEL 1: inverter, voltage open loop\n"
...
class BoardPinPage(QWizardPage):
    def __init__(self):
        super().__init__()
        self.setTitle("步骤2：设置板级引脚")

        # 初始化页面布局
        layout = QVBoxLayout()

        # 添加标签和输入框
        self.launchpad_label = QLabel("请输入Launchpad引脚")
        self.launchpad_input = QLineEdit()
        self.launchpad_input.setPlaceholderText("例如：0")

        self.gmp_iris_label = QLabel("请输入GMP-Iris引脚")
        self.gmp_iris_input = QLineEdit()
        self.gmp_iris_input.setPlaceholderText("例如：1")

        layout.addWidget(self.launchpad_label)
        layout.addWidget(self.launchpad_input)
        layout.addWidget(self.gmp_iris_label)
        layout.addWidget(self.gmp_iris_input)

        self.setLayout(layout)

    def validatePage(self):
        return bool(self.launchpad_input.text() and self.gmp_iris_input.text())
    
    def get_data(self):
        return self.launchpad_input.text(), self.gmp_iris_input.text()

class ControllerBasicPage(QWizardPage):
    def __init__(self):
        super().__init__()
        self.setTitle("步骤3：设置控制器基本参数")
...
class CtrlSettingsPage(QWizardPage):
    def __init__(self):
        ...
        self.SPLL_Close_loop_criteria = QLabel("请输入SPLL关闭循环准则：")
        self.SPLL_Close_loop_criteria_input = QLineEdit()
        self.SPLL_Close_loop_criteria_input.setPlaceholderText("例如：0.01")
        ...
        layout.addWidget(self.SPLL_Close_loop_criteria)
        layout.addWidget(self.SPLL_Close_loop_criteria_input)
    ...
    def get_data(self):
        return {
            ...
            "SPLL_Close_loop_criteria": self.SPLL_Close_loop_criteria_input.text()
        }

    def validatePage(self):
        return bool(self.use_discrete_pid_input.isChecked() and
                    self.enable_adc_calibration_input.isChecked() and
                    self.SPLL_Close_loop_criteria_input.text())
...
class ConfigWizard(QWizard):
    ...
    def on_finished(self, result):
        ...
        if result == QDialog.DialogCode.Accepted:
            ...
            ctrlBaseData = self.page(4).get_data()
            ctrlSettingsData = self.page(5).get_data()

            # 生成头文件内容
            self.generate_ctrl_settings_header(targetFolder, debugData, basicData, hardwareData, ctrlBaseData, ctrlSettingsData)
    ...
    def generate_ctrl_settings_header(self, targetFolder, debugData, basicData, hardwareData, encoderData, ctrlBaseData, ctrlSettingsData):
        ...
        header_content += "#define LAUNCHPAD %s\n"
        header_content += "#define GMP_IRIS %s\n\n"
        header_content += "#define BOARD_SELECTION GMP_IRIS\n\n"
        ...
        header_content += "// 网侧传感器\n"
        header_content += "#define CTRL_GRID_CURRENT_SENSITIVITY (HARMONIA_3PH_LC_FILTER_PH_CURRENT_SENSITIVITY_MV_A*0.001f)\n"
        header_content += "#define CTRL_GRID_CURRENT_BIAS (HARMONIA_3PH_LC_FILTER_PH_CURRENT_ZERO_BIAS_V)\n"
        header_content += "#define CTRL_GRID_VOLTAGE_SENSITIVITY (HARMONIA_3PH_LC_FILTER_PH_VOLTAGE_SENSE_GAIN)\n"
        header_content += "#define CTRL_GRID_VOLTAGE_BIAS (HARMONIA_3PH_LC_FILTER_PH_VOLTAGE_SENSE_BIAS_V)\n\n"
        header_content += "// 逆变器传感器\n"
        ...
        header_content += "#define CTRL_SPLL_EPSILON ((float2ctrl(%s)))         // 锁相环误差阈值\n\n"
        
        header_content += "#endif // _FILE_CTRL_SETTINGS_H_\n"
        ...
        filled_content = header_content % (
            debug_type,
            ctrl_start_delay, ctrl_freq, pwm_depth, pwm_deadband, ctrl_adc_ref,
